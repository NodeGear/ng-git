	...
	async.waterfall([
		function (done) {
			models.RSAKey.findOne({
				...
				if (!rsaKey) {
					...
					send_git_response(msg, "SSH Key Not Found", true);

					return done('SSH Key Not Found');
				}

				done(null, rsaKey);
			});
		},
		function (rsaKey, done) {
			models.User.findOne({
				_id: rsaKey.user,
				disabled: false
				...
				if (!user) {
					...
					send_git_response(msg, "Unknown User..", true);
		...
		function (user, done) {
			...
			models.App.findOne({
				...
				done(null, user, app, repo);
			});
		},
		function (user, app, repo, done) {
			if (app != null) {
				return done(null, user, app, repo);
			}

			console.log("--> App does not exist", msg);

			// Lookup a friendly-looking url
			var attempt = 0;
			var nameUrl = repo.replace(/\W+/g, '-').trim().toLowerCase();
			async.retry(10, function (cb, results) {
				...
				var app = new models.App({
					...
					script: "index.js",
					nameUrl: url,
					location: 'git@nodegear.io:'+repo+'.git',
					branch: 'master'
				});
				...
				async.retry(10, function (cb, results) {
					...
					models.AppDomain.findOne({
						domain: url + '-' + user.username + '.ngapp.io'
					...
					send_git_response(msg, "Creating NodeGear App Domain: " + url);
		...
		function (user, app, repo, done) {
			...
			models.AppProcess.find({
				...
				done(null, user, app, repo, processes);
			});
		},
		function (user, app, repo, processes, done) {
			...
			models.Server.findOne({
				...
				if (!server) {
					...
					done('Cannot find a suitable server');
				...
				models.App.find({
					user: user._id,
					deleted: false
				}).select('_id').exec(function (err, apps) {
					// Count processes
					var appids = [];
					...
					models.AppProcess.count({
						...
						if (processCount >= user.appLimit) {
							console.log("User over limit", user.appLimit);
							send_git_response(msg, "You have exceeded your Application Limit. An app has been created, contact us to raise the limit and then create a process.", true);
							
							done('user over limit');
