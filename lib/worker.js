...
function worker (affected_key, callback) {
	var path = gitolite+'keydir/'+affected_key.key._id+(new Date(affected_key.key.created).getTime())+'.pub';
...
\n";
	...
	RW+ =";
					...
					for (var i = 0; i < keys.length; i++) {
						...
						base += " " + key._id + (new Date(key.created).getTime());
					}
			...
			fs.writeFile(gitolite+'conf/gitolite.conf', base, function(err) {
				...
				// Update git..
				var git_update = spawn(config.path+'/scripts/updateGitolite.sh', [gitolite]);
